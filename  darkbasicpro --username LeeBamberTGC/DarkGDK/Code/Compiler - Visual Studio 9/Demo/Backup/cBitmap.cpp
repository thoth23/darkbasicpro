...
bool bitmap::save(LPDIRECT3DSURFACE9 surface, char *filename)
{
	...
	for(int y=0; y<height; y++) 
	{
		...
		{
			DWORD* dwPix = ( DWORD* ) rect.pBits;

			//getcolor(x, y, color[0], color[1], color[2]);

			DWORD dwCol = dwPix [ x + y * rect.Pitch / 4 ];

			color [ 0 ] = ( unsigned char ) ((( dwCol ) >> 16 ) & 0xff );
			color [ 1 ] = ( unsigned char ) ((( dwCol ) >>  8 ) & 0xff );
			color [ 2 ] = ( unsigned char ) ((( dwCol )       ) & 0xff );
...
bool bitmap::save(char *filename)
{
	...
	for(int y=0; y<height; y++) 
	{
		for(int x=0; x<width; x++) 
		{
			...
			//color [ 0 ] = ((( dwCol ) >> 16 ) & 0xff );
			//color [ 1 ] = ((( dwCol ) >>  8 ) & 0xff );
			//color [ 2 ] = ((( dwCol )       ) & 0xff );
