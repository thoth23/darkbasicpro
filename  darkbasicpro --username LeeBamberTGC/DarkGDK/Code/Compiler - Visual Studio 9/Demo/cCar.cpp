...
cCar::cCar ( int iID, char* szFile, float fX, float fY, float fZ )
{
	...
	m_fWheelNominalSpringLength     = new sData (     6.0f,     6.0f,     6.0f,     6.0f );
	m_fWheelSpringStiffness         = new sData ( 45000.0f, 45000.0f, 45000.0f, 45000.0f );
	m_fWheelSpringDamping           = new sData ( 90000.0f, 90000.0f, 90000.0f, 90000.0f );
...
void cCar::ApplyBrakes ( void )
{
	...
	if ( m_bReverse == false )
	{
		...
		//dbMeqCarSetWheelBrakeTorque ( m_iID, 0, 1 );
		//dbMeqCarSetWheelBrakeTorque ( m_iID, 1, 1 );
		//dbMeqCarSetWheelBrakeTorque ( m_iID, 2, 1 );
		//dbMeqCarSetWheelBrakeTorque ( m_iID, 3, 1 );
