...
void MakeGame ( LPSTR pStyle, LPSTR pHero, LPSTR pEnemy, LPSTR pStory )
{
	...
	// Compare difference
	int iDifference = (int)abs ( (int)dwCurrentESP - (int)dwRecordedESP );
	if ( iDifference > 12 )
...
void LN ( void )
{
	...
	DWORD dwRecordedLINE = *pLINEPtr;
	char pText[10240];
	wsprintf ( pText, "Line Number : %d\n", dwRecordedLINE);
}

//
// HOW TO : PERFORM CHECLIST
//
void CL ( void )
{
	...
	for ( int i = 0; i < g_pGlob->checklistqty; i++ )
	{
		...
		strcpy ( g_pGlob->checklist [ i ].string, "item" );
	}
}
