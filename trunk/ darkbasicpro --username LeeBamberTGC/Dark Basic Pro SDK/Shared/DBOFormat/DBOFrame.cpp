...
DARKSDK_DLL void GetMatrixFromKey ( D3DXMATRIX* pResultMatrix, sAnimation* pAnim, float fTime )
{
	...
	for ( int i = dwKey; i < ( int ) dwKeyMax; i++ )
	{
	...
	/* crashes because of dirst condition sometimes (exe run)
	// need to locate key quickly based on last position (most likely near here)
	while ( 1 )
	{
		// if between current and next, keyframe has been found
		if ( pAnim->pMatrixKeys [ dwKey ].dwTime <= fTime 
		&&   pAnim->pMatrixKeys [ dwKey+1 ].dwTime > fTime )
			break;

		// if hit end, this is our keyframe
		if ( dwKey>=pAnim->dwLastMatrixKey-1 ) { dwKey=0; continue; }

		// if next one is less or equal to time, go to next one
		if ( pAnim->pMatrixKeys [ dwKey+1 ].dwTime <= fTime )
			{ dwKey++; continue; }

		// if current one is greater than where I should be, move back
		if ( pAnim->pMatrixKeys [ dwKey ].dwTime > fTime ) dwKey--;

		// if hit zero, this is our keyframe
		if ( dwKey==0 )
			break;
	}
