...
void CMatrixManager::Update ( int iMode )
{
	...
	for ( int temp = 0; temp < m_List.m_count; temp++ )
	{
		...
		if ( iMode==0 && ptr->bRenderAfterObjects==true ) continue;
		if ( iMode==1 && ptr->bRenderAfterObjects==false ) continue;
...
//			// z write enable on
		...
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_ADDRESSU, ptr->iTextureMode );
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_ADDRESSV, ptr->iTextureMode );
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MAGFILTER, ptr->iFilter );	// mip mapping
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MINFILTER, ptr->iFilter );	// mip mapping
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MIPFILTER, ptr->iMipFilter );	// mip mapping
...
//		ptr->pD3D->SetTextureStageState ( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE );
		...
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MAGFILTER, 2 );	// mip mapping
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MINFILTER, 2 );	// mip mapping
		ptr->pD3D->SetSamplerState ( 0, D3DSAMP_MIPFILTER, 2 );	// mip mapping
