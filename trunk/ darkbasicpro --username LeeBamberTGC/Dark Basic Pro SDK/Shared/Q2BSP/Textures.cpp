...
LPDIRECT3DTEXTURE9 _Textures::Create_From_RAW_Data ( byte* data, UINT width, UINT height, int bpp )
{
	...
	LPDIRECT3DSURFACE9 lpSur1, lpSur2, lpSur3;
	...
	m_pD3D->CreateOffscreenPlainSurface ( pDesc.Width, pDesc.Height, pDesc.Format, D3DPOOL_SCRATCH, &lpSur2, NULL );
	
	D3DXLoadSurfaceFromSurface ( lpSur2, NULL, NULL, lpSur1, NULL, NULL, D3DX_DEFAULT, 0 );

	LPDIRECT3DTEXTURE9 temp_tex;
	D3DXCreateTexture ( m_pD3D, pDesc.Width, pDesc.Height, 1, NULL, pDesc.Format, D3DPOOL_SYSTEMMEM, &temp_tex );
	temp_tex->GetSurfaceLevel ( 0, &lpSur3 );
	//m_pD3D->UpdateSurface ( lpSur2, NULL, lpSur3, NULL );
	m_pD3D->StretchRect ( lpSur2, NULL, lpSur3, NULL, D3DTEXF_NONE );

	m_pD3D->UpdateTexture ( temp_tex, tex );
...
int _Textures::Load_LightMap_By_Data_Q2 ( byte* data, UINT width, UINT height, int bpp )
{
	...
	if ( bpp == 24 )
	{
		for ( int x = 0; x < size; x += 3 )
		{
			...
			tex_data [ x + 0 ] = data [ x + 0 ];
			tex_data [ x + 1 ] = data [ x + 1 ];
			tex_data [ x + 2 ] = data [ x + 2 ];
		}
	}
	...
	*/
	...
	D3DXCreateTexture ( m_pD3D, width, height, 1, NULL, D3DFMT_R8G8B8, D3DPOOL_MANAGED, &LightMap [ n ].lpD3DTex );

	LightMap [ n ].lpD3DTex->LockRect ( 0, &d3dlr, 0, 0 );
	{
		...
		for ( DWORD y = 0; y < ( DWORD ) height; y++ )	
		{
			...
			{
				//*pPix++ = D3DCOLOR_ARGB ( 255, data [ iStart + 0 ], data [ iStart + 1 ], data [ iStart + 2 ] );
				//*pPix++ = D3DCOLOR_ARGB ( 255, data [ iStart++ ], data [ iStart++ ], data [ iStart++ ] );

				*pPix++ = D3DCOLOR_XRGB ( tex_data [ iStart + 0 ], tex_data [ iStart + 1 ], tex_data [ iStart + 2 ] );
				//*pPix++ = D3DCOLOR_XRGB ( 255, 0, 0 );
				iStart += 3;
			}

			iLine--;
			iStart = width * iLine * 3;
		}
	}

	LightMap [ n ].lpD3DTex->UnlockRect ( NULL );
	*/
	////////////////////////
