...
cCar::cCar ( int iID, char* szFile, float fX, float fY, float fZ )
{
	...
	m_fDriveConstant0			= 700.0f    * m_fScale * m_fScale;			// control how powerful the engine is
...
void cCar::Drive ( void )
{
	...
	if ( dbKeyState ( 17 ) || AccelOn == true )
	{
		// state of acceleration
		if ( m_bApplyAcceleration )
			ApplyAcceleration ( );
			if ( dbSoundPlaying ( 6 ) == 0 )
			{
				dbPlaySound ( 6 );
			}
	}
	else if ( dbKeyState ( 31 ) || BrakeOn == true )
	{
		// state of braking
		if ( m_bApplyBrakes )
			ApplyBrakes ( );
	...
	else
	{
		...
		if ( timeGone() > 40.0 )
		{
			m_fMotorInput -= 0.005f;
			timeReset();
		}
		if ( m_fMotorInput < 0.0f )
		{
			m_fMotorInput = 0.0f;
...
void cCar::ApplyAcceleration ( void )
{
	...
	if ( timeGone() > 40.0 )
	{
		m_fMotorInput += 0.0025f;

		if ( m_fMotorInput < 0.25f )
			m_fMotorInput = 0.25f;
...
void cCar::ApplyBrakes ( void )
{
	...
	if ( m_bReverse == false )
	{
		...
		//m_fWheelBrakeTorque->a = 1;
		//m_fWheelBrakeTorque->b = 1;
		//m_fWheelBrakeTorque->c = 1;
		//m_fWheelBrakeTorque->d = 1;
